...
def readerversion_test(
    readerlibrarytest, com, rdversion, rdvermxlen, apiversion, apivermaxlen
):
    stat = readerlibrarytest.JT_ReaderVersion(
        ctypes.c_int(com),
        rdversion,
        ctypes.c_int(rdvermxlen),
        apiversion,
        ctypes.c_int(apivermaxlen),
    )
    return stat
...
class VFJReader:
    ...
    def __init__(self, systemtype, cn, pointer, dllpath="./DLL"):
        ...
        # print("加载的动态库是："+str(ctypes.LibraryLoader.__repr__))JT_LEDDisplay
    ...
    # 打开读写器
    def openreader(self):
        openstat = self.readerlibrary.JT_OpenReader(
            ctypes.c_int(0), ctypes.c_char_p(self.pointer)
        )
    ...
    def leddisplay(self, nHandle, red, green, blue):
        stat = self.readerlibrary.JT_LEDDisplay(
            ctypes.c_int(nHandle),
            ctypes.c_int(red),
            ctypes.c_int(green),
            ctypes.c_int(blue),
        )
    ...
    def audiocontrol(self, nHandle, time, voice):
        stat = self.readerlibrary.JT_AudioControl(
            ctypes.c_int(nHandle), ctypes.c_int(time), ctypes.c_int(voice)
        )
        return stat

    def getstatus(self, nHandle, statuscode):
        stat = self.readerlibrary.JT_GetStatus(
            ctypes.c_int(nHandle), ctypes.byref(statuscode)
        )
    ...
    def readerversion(self, nHandle, rdversion, rdvermxlen, apiversion, apivermaxlen):
        ...
        stat = self.readerlibrary.JT_ReaderVersion(
            ctypes.c_int(nHandle),
            rdversion,
            ctypes.c_int(rdvermxlen),
            apiversion,
    ...
    def getstatusmsg(self, ststuscode, ststusmag, maglen):
        stat = self.readerlibrary.JT_GetStatusMsg(
            ctypes.c_int(ststuscode), ststusmag, ctypes.c_int(maglen)
        )
        return stat

    def opencard(self, nHandle, cardplace, cardsn):
        stat = self.readerlibrary.JT_OpenCard(
            ctypes.c_int(nHandle), ctypes.byref(cardplace), cardsn
        )
        return stat
    ...
    def cpucommandself(self, nHandle, command, lencom, reply, lenrep):
        ...
        stat = self.readerlibrary.JT_CPUCommand(
            ctypes.c_int(nHandle),
    ...
    def samreset(self, nHandle, nSockID, protocoltype):
        stat = self.readerlibrary.JT_SamReset(
            ctypes.c_int(nHandle), ctypes.c_int(nSockID), ctypes.c_int(protocoltype)
        )
        return stat

    def samcommand(self, nHandle, nSockID, command, lencom, reply, lenrep):
        stat = self.readerlibrary.JT_SamCommand(
            ctypes.c_int(nHandle),
            ctypes.c_int(nSockID),
            ctypes.c_char_p(command),
            ctypes.c_int(lencom),
            reply,
            ctypes.byref(lenrep),
    ...
    def selectRWUnit(self, nHandle, Unit):
        stat = self.readerlibrary.JT_SelectRWUnit(
            ctypes.c_int(nHandle), ctypes.c_ubyte(Unit)
        )
        return stat

    def setScanRWUnits(self, nHandle, Unit):
        stat = self.readerlibrary.JT_SetScanRWUnits(
            ctypes.c_int(nHandle), ctypes.c_ubyte(Unit)
        )
        return stat
...
if __name__ == "__main__":
    try:
        ...
        readerversion = reader.readerversion(
            openstat, rdversion, len(rdversion), apiversion, len(apiversion)
        )
        ...
        aa = reader.samcommand(
            openstat, 1, command2, len(command2), psamreply, psamlenrep
        )
        print(aa)
