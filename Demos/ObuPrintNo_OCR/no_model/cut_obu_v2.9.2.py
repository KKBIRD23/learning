...
VERSION = "v2.9.2"
"""
自动调优：python cut_obu_v2.9.2.py --img_path 3.jpg --auto_tuning
最佳参数手动验证：python cut_obu_v2.9.2.py --img_path 3.jpg --upper_black_v 55 --morph_size 7 --min_area 500 --min_w 70 --min_h 30
"""
...
def process_image():
    ...
    if args.auto_tuning:
        ...
        for idx, (upper_v, morph_size, min_area, (min_w, min_h)) in enumerate(param_combinations, 1):
            ...
            cmd = f"--upper_black_v {upper_v} --morph_size {morph_size} --min_area {min_area} --min_w {min_w} --min_h {min_h}    本组合cnt={cnt}"
            if 20 <= cnt <= 22:  # 调整目标范围
                output_path = os.path.join(
                    SELECTED_DIR,
                    f"marked_upper{upper_v}_morph{morph_size}_area{min_area}_w{min_w}_h{min_h}_cnt{cnt}.jpg"
                )
            ...
            with open(log_file, "a", encoding="utf-8") as f:
                f.write(f"组合 {idx}/{total_combinations}: {cmd}\n")
        with open(log_file, "a", encoding="utf-8") as f:
        ...
        output_path = os.path.join(
            OUT_DIR,
            f"marked_upper{args.upper_black_v}_morph{args.morph_size}_area{args.min_area}_w{args.min_w}_h{args.min_h}_cnt{cnt}.jpg"
        )
        cv2.imwrite(output_path, img)
        with open(log_file, "a", encoding="utf-8") as f:
            cmd = f"--upper_black_v {args.upper_black_v} --morph_size {args.morph_size} --min_area {args.min_area} --min_w {args.min_w} --min_h {args.min_h}    本组合cnt={cnt}"
            f.write(f"手动模式: {cmd}\n")
